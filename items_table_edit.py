...
def add( database, my_cursor ):
    ...
    new_item_starting_durability = user_in.get_int("\nEnter the starting durability of your item:\n", "\nInvalid input make sure your item's durability is entered as an integer.\n")
#adding the new item
    my_cursor.execute("INSERT INTO items (name, rarity,starting_durability) VALUES( '" + new_item_name + " '," + str(new_item_rarity) + " ," + str(new_item_starting_durability) + ");")
    database.commit()
...
def modify( database, my_cursor ):
    #Prompts user for item id and item modifications and applies them to the database
    ...
    while(True):
        ...
        #Check if the item exists:
        my_cursor.execute("SELECT * FROM items WHERE item_id = " + str(item_id) +";")
        target_item = my_cursor.fetchone()
        if target_item is not None:
            print("Item " + str(item_id) + " found!")
            break
        else:
            print("That item does not exist. Try again")
    ...
    while(True):
        ...
        match user_input:
            ...
            case 2:
                ...
                database.commit()
                print("Item_id: " + str(item_id) + " rarity updated to: " + str(new_item_rarity))
                break
            case 3:
                new_item_starting_durability = user_in.get_int("\nEnter the new item starting durability:\n", "That is not a valid input try inputting an integer")
                my_cursor.execute("UPDATE items SET starting_durability = "+ str(new_item_starting_durability) + " WHERE item_id = " + str(item_id) +";")
                database.commit()
                print("Item_id: " + str(item_id) + " starting durability updated to: " + str(new_item_starting_durability))
                break
            case _:
